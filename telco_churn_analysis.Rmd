...
telco %>% replace(is.na(.), 0) %>%
   summarise(sum(TotalCharges))

summary(monthlyfeedf$TotalCharges)
...
tenure_plawt <- ggplot(tenuredf, aes(x = factor(cohort), fill = factor(Churn))) +
  geom_bar(position = "dodge") +
  scale_y_continuous("Customers")+
  scale_fill_manual("Churn", values = val)+
  scale_x_discrete("Tenure") 

prop.table(table(tenuredf$cohort, tenuredf$Churn),1)
...
fit1 <- rpart(Churn ~ Partner + Dependents + Fiber_Optic + Year1 + Electronic_Check + MonthlyCharges, data = tree_data, method="class")
...
Prediction <- predict(fit1, tree_data, type = "class")
...
glm.test <- glm(Churn ~ Partner + Dependents + Fiber_Optic + Electronic_Check + MonthlyCharges, data = test, family = binomial)
...
ggplot(training, aes(x=MonthlyCharges, y=Churn)) + geom_point() + 
  stat_smooth(method="glm", method.args=list(family="binomial"), se=FALSE)


ggplot(training, aes(x=Fiber_Optic, y=Churn)) + geom_point() + 
  stat_smooth(method="glm", method.args=list(family="binomial"), se=FALSE)
